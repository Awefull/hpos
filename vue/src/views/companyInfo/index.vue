                    ...
                    <div class="option-content">
                      <div class="option-list">
                        <div>
                          <li
                            class="liMenu"
                            :class="idxServiceRole==indexServiceRole?'serviceRole':''"
                            @click="selectServiceRole(itemServiceRole,idxServiceRole)"
                            v-for="(itemServiceRole,idxServiceRole) in serviceRole"
                            :key="idxServiceRole"
                          >{{itemServiceRole}}</li>
                        </div>
                      </div>
        ...
        <div class="white-bgcolor">
          <div class="item-container-2Txk_">
                ...
                <el-col :span="24" v-for="o in list" :key="o"  class="padding-top20">
                  <el-card :body-style="{ padding: '5px' }">
                    <div class="icon-content-3GTl0">
                      <a @click="companyInfo(o.projectName,o.unitName)">
                    ...
                    <div class="body-content-2y6ks">
                      <div class="info-body-34ARl">
                        <div class="title-2q5sH">
                          <a @click="companyInfo(o.projectName,o.unitName)" class="name-UUeBS">
                            <!-- <span  class="no-3TiXA">NO.</span> -->
                            <div class="evaluate" style="font-size: 16px;">
                              <label>项目名称：</label>
                              <label >{{o.projectName}}</label>
                            </div>
                          </a>
                        </div>
                        <div>
                        ...
                        <div class="time-2b2Sj">
                          <span>13 小时前发布</span>
                          <span>{{o.applyNum}}人报名</span>
                        </div>
                        <div class="evaluate">
                          <label>项目描述：</label>     
                          <span>{{o.projectDescription}}</span>
                        </div>
                      </div>
                    </div>
                  </el-card>
            ...
            <el-pagination
              ...
              :current-page="companyMessage.pageNum"
              :page-size="companyMessage.pageRow"
              :total="totalCount"
...
export default {
  ...
  methods: {
    ...
    selectServiceRole(itemServiceRole, idxServiceRole) {
      this.indexServiceRole = idxServiceRole;
      this.selectedItems.serveRole = itemServiceRole; 
      this.api({
        url: "/companyInfo/filterCompany",
        method: "post",
        data: this.selectedItems
    ...
    getList() {
      ...
      this.api({
        url: "/companyInfo/listCompanyInfo",
        method: "get",
        params: this.companyMessage
      }).then(data => {
          this.listLoading = false;
          this.list = data.list;
          this.totalCount = data.totalCount;
    ...
    companyInfo(projectName,unitName) {
      this.demandIndex.projectName = projectName;
      this.demandIndex.companyName = unitName;
      this.api({
    ...
    handleSizeChange(val) {
      //改变每页数量
      this.companyMessage.pageRow = val;
      this.handleFilter();
    },
    handleCurrentChange(val) {
      //改变页码
      this.companyMessage.pageNum = val;
      this.getList();
    },
    handleFilter() {
      //查询事件
      this.companyMessage.pageNum = 1;
      this.getList();
    },
    getIndex($index) {
      //表格序号
      return (
        (this.companyMessage.pageNum - 1) * this.companyMessage.pageRow +
        $index +
        1
      );
    }
